...
const formSchema = z.object({
  firstName: z
    ...
    .regex(/^[a-zA-Z\s]+$/,{message : "Harus menggunakan huruf",}),
    lastName: z
    .string()
    .regex(/^[a-zA-Z\s]+$/, { message: "Harus menggunakan huruf" })
    .optional()
  ...
  username: z
    ...
    .regex(/[^a-zA-Z0-9_]/, {message : 'Username hanya boleh menggunakan huruf, angka, atau _.'}),
  email : z
    .string()
    .email({
  ...
  password: z
    .string().min(8, { message: "Password harus memiliki minimal 8 karakter." })
    .regex(/^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, {
      message:
        "Password harus memiliki huruf besar, angka, dan karakter.",
    }),
  password_confirmation: z.string().min(8, { message: "Password dan Confirm Password tidak sama." }),
}).refine((data) => data.password !== data.username, {
  message: "Password tidak boleh sama dengan username.",
  path: ["password"],
}).refine((data) => data.password === data.password_confirmation, {
  message: "Password dan Confirm Password tidak sama.",
  path: ["password_confirmation"], // Error akan muncul di field password_confirmation
})
        ...
        <FormField
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild className="flex">
              <button type="submit" className={`rounded-2xl bg-purple-800 text-white items-center justify-center w-full h-14 font-bold !mt-4 ${ processing ? 'cursor-not-allowed' : ''} ${!form.formState.isValid ? 'opacity-50' : ''}`} disabled={!form.formState.isValid || processing}>{processing ?  <Spinner className='animate-spin'/> : "Daftar"}</button>
            </AlertDialogTrigger>
